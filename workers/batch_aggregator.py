...
from sqlalchemy import create_engine, select, func, delete, cast, Integer, Date, extract
from sqlalchemy.orm import sessionmaker
from app.models.orm import WindowReport, DailyAverage, WeeklyAverage, MonthlyAverage, YearlyAverage
...
def calculate_averages_for_date(target_date: date, patient_id: Optional[int] = None):
    ...
    with SessionLocal() as db:
        try:
            #dailyAverage
            daily_conditions = [
                cast(WindowReport.timestamp, Date) == target_date,
            ...
            ).where(*daily_conditions).group_by(WindowReport.patient_id)
            ...
            #weeklyAverage
            weekly_conditions = [
                DailyAverage.report_date >= start_of_week,
                DailyAverage.report_date <= end_of_week
            ]
            if patient_id is not None:
            ...
            ).where(*weekly_conditions).group_by(DailyAverage.patient_id)
            ...
            #monthlyAverage
            monthly_conditions = [
                func.to_char(DailyAverage.report_date, 'YYYY-MM') == target_month_str
            ]
            if patient_id is not None:
            ...
            stmt_monthly = select(
                ...
                func.sum(DailyAverage.anomaly_count).label("anomaly_count")
            ).where(*monthly_conditions).group_by(DailyAverage.patient_id)
            ...
            #yearlyAverage
            yearly_conditions = [
                extract('year', DailyAverage.report_date) == target_year_int
            ]
            if patient_id is not None:
            ...
            stmt_yearly = select(
                ...
                func.sum(DailyAverage.anomaly_count).label("anomaly_count")
            ).where(*yearly_conditions).group_by(DailyAverage.patient_id)
            ...
            if patient_id is None:
                ...
                del_stmt = delete(WindowReport).where(
                    cast(WindowReport.timestamp, Date) < cutoff_date
                )
