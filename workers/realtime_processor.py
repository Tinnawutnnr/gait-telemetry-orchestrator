...
class GaitSystem:
    ...
    def _extract_params(self, sig):
        ...
        max_val = np.max(sig)
        if max_val < 1.0: return None # สัญญาณเบาเกิน (นั่งเฉยๆ)
        ...
        if len(valid_hs) < 3: return None # ก้าวน้อยเกินไป (Noise/หยุดเดิน)
        ...
        if len(stride_times) < 2: return None
    ...
    def _train_model(self):
        if len(self.normal_windows) < 2: return # ต้องมีอย่างน้อย 2 ถึงจะเทรนได้
