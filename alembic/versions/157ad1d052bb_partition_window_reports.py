...
def upgrade() -> None:
    ...
    op.execute(
        "CREATE TABLE window_reports_2026_03 PARTITION OF window_reports "
        "FOR VALUES FROM ('2026-03-01') TO ('2026-04-01');"
    )
    op.execute(
        "CREATE TABLE window_reports_2026_04 PARTITION OF window_reports "
        "FOR VALUES FROM ('2026-04-01') TO ('2026-05-01');"
    )
    op.execute(
        "CREATE TABLE window_reports_2026_05 PARTITION OF window_reports "
    ...
    op.execute("""
        CREATE TABLE anomaly_logs (
            ...
            CONSTRAINT fk_anomaly_logs_window_reports FOREIGN KEY (window_id, timestamp)
                REFERENCES window_reports(window_report_id, timestamp),
    ...
    op.execute(
        "CREATE TABLE anomaly_logs_2026_03 PARTITION OF anomaly_logs FOR VALUES FROM ('2026-03-01') TO ('2026-04-01');"
    )
    op.execute(
        "CREATE TABLE anomaly_logs_2026_04 PARTITION OF anomaly_logs FOR VALUES FROM ('2026-04-01') TO ('2026-05-01');"
    )
    op.execute(
        "CREATE TABLE anomaly_logs_2026_05 PARTITION OF anomaly_logs FOR VALUES FROM ('2026-05-01') TO ('2026-06-01');"
    )
